...
- 支持英文应用名直接匹配
- **多音字**：一个字的常见读音都会参与匹配，例："贝壳找房"输入 `beike`（T9 数字 `23453`）或 `beiqiao`（`237426`）都能搜到；"重庆"`chongqing`/`zhongqing`、"银行"`yinhang`、"音乐"`yinyue`、"便宜"`pianyi` 等同理

### 快捷手势
- **上滑数字键**：启动搜索列表对应位置的应用
- **长按数字键**：启动该键绑定的应用
- **默认长按绑定**：长按 `1` = Clash、`7` = 微信、`8` = Twitter、`9` = 知乎。首次运行（含升级后首次启动）自动绑定，只绑定已安装的应用，且不会覆盖你自己设置过的绑定；也可以在「数字键绑定应用」里随时改
...
- 定时刷新应用列表（30 分钟间隔）
- 分类筛选（最近搜索 / 最近使用 / 最近安装 / 社交 / 影音 / 交通出行 / 实用工具 / 游戏 / 购物 / 理财），默认打开「最近使用」页
  - **最近搜索**：按时间倒序列出通过 T9 搜索启动过的应用，长按该 chip 可清空历史
  - **最近使用**：启动过的应用按最近启动时间倒序
  - **最近安装**：在「最近更新范围」设置的时间范围内安装的应用
- 分类管理：智能分类（最近搜索/最近使用/最近安装）与「主界面」不可删除，智能分类可改名、排序、设为默认打开页；其余分类支持添加、删除、关键词/手动编辑，可指定默认打开的分类
